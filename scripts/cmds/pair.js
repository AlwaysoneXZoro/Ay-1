...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData }) {
    ...
    var senderGender = (await usersData.get(senderID)).gender;
    var possibleIDs = [];
    for (let id of listUserID) {
      let userGender = (await usersData.get(id)).gender;
      if (senderGender === 2 || userGender === 2) {
        possibleIDs.push(id);
      } else if ((senderGender === 0 && userGender === 1) || (senderGender === 1 && userGender === 0)) {
        possibleIDs.push(id);
      }
    }
    var id = possibleIDs[Math.floor(Math.random() * possibleIDs.length)];
